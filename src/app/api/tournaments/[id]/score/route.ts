...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [match] = await sql<any[]>`select * from matches where tournament_id=${id} and id=${matchId} limit 1`;
    if (!match) return NextResponse.json({ error: 'Match not found' }, { status: 404 });

    // Update match
    const winner = Number(scoreA) === 21 ? 'A' : 'B';
    await sql`update matches set score_a=${scoreA}, score_b=${scoreB}, completed=true, winner_team=${winner} where tournament_id=${id} and id=${matchId}`;
    ...
    // Fetch all players
    const players = await sql<any[]>`select id from players where tournament_id=${id}`;
    for (const p of players) {
      ...
      await sql`update players set total_score=${total} where tournament_id=${id} and id=${p.id}`;
    }

    // Check if round is complete and possibly advance current round
    const currentRoundMatches = await sql<{ completed: boolean }[]>`select completed from matches where tournament_id=${id} and round=${round}`;
    const roundCompleted = currentRoundMatches.every((m: { completed: boolean }) => m.completed === true);
    if (roundCompleted) {
      const [trow] = await sql<any[]>`select current_round from tournaments where id=${id}`;
      const currentRound = Number(trow?.current_round || 1);
      if (currentRound === round && currentRound < 7) {
        await sql`update tournaments set current_round=${currentRound + 1} where id=${id}`;
      }
    }
